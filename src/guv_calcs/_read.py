...
def file_to_zone(file_path):
    ...
    if 'Data format notes' in lines[0]:
        ...
        y1, y2 = yp[0], yp[-1] 
        z1, z2 = zp[0], zp[-1]
        zone = CalcVol(zone_id=path.stem, x1=x1, x2=x2, y1=y1, y2=y2, z1=z1,z2=z2,
                      x_spacing=x_spacing, y_spacing=y_spacing, z_spacing=z_spacing,dose=dose,offset=False)
        zone.values = values
        zone.coords = coords
    else:
        values, coords, points, height, ref_surface = read_irradiance(lines)
        ...
        y1, y2 = yp[0], yp[-1] 
        try:
            zone = CalcPlane(zone_id=path.stem, x1=x1, x2=x2, y1=y1, y2=y2, height=height, 
                      x_spacing=x_spacing, y_spacing=y_spacing,
                      ref_surface=ref_surface, offset=False)     
            zone.values = values
            zone.coords = coords
