...
def calculate_values(
    coords: np.array, lamps: list, room: dict, fov80=False, horiz=False, vert=False
):
    ...
    for lamp in lamps:
        ...
        values = np.array(
            [
                get_intensity(theta, phi, valdict) / r ** 2
...
def calculate_fluence(lamps: list, room: dict):
    ...
    points = [
        ...
        for val, div in zip((x, y, z), (xdiv, ydiv, zdiv))
    ]
...
def calculate_plane(
    lamps, room, height=1.9, div=0.1, fov80=False, horiz=False, vert=True, dose=True
):
    """
